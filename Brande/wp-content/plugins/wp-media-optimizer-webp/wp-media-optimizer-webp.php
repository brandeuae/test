...
function wpmowebp_options_settings(){
	...
	if(isset($_REQUEST['item_name']) and isset($_REQUEST['token'])) {
		...
		$response = wp_remote_get("http://www.francescosganga.it/plugins/validate-token.php?item_name={$_REQUEST['item_name']}&token={$_REQUES['token']}");
...
function wpmowebp_options_abovethefold() {
	...
	<hr />
	<strong style="color: blue"><?php if($atf == "1") _e("Above the Fold feature is currenctly ENABLED"); else _e("Above the Fold feature is currenctly DISABLED"); ?></strong><br />
	<br />
...
function wpmowebp_filter_content($content) {
	...
	if(wpmowebp_detect_browser() != false) {
		...
		$content = preg_replace_callback("/{$prot}:\/\/({$withoutwww}|{$withwww})\/([^\/]+)\/uploads\/([^\/]+)\/([^\/]+)\/([A-zÀ-ú0-9-.]+).(png|jpg|jpeg)/", function($matches) {
			if(!file_exists(WP_CONTENT_DIR . "/wpmowebp/{$matches[2]}/uploads/{$matches[3]}/{$matches[4]}/{$matches[5]}.webp")) {
				if(!wpmowebp_imagetowebp($matches[2], "/uploads/{$matches[3]}/{$matches[4]}/{$matches[5]}.{$matches[6]}")) {
					return $matches[0];   
				}
			}
			
			return WP_CONTENT_URL . "/wpmowebp/{$matches[2]}/uploads/{$matches[3]}/{$matches[4]}/{$matches[5]}.webp";
		}, $content);
	}
	//lady load
